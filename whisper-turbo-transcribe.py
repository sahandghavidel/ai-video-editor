#!/usr/bin/env python3
"""
Whisper Turbo transcription script that can be called from Node.js
Takes a media URL as argument and returns JSON transcription
Uses the turbo Whisper model - optimized large-v3 with ~8x speed and near-equivalent accuracy
"""
...
def transcribe_with_whisper_turbo(audio_path: str) -> dict:
    ...
    try:
        ...
        # Load the turbo model (optimized large-v3, ~8x faster)
...
def main():
    if len(sys.argv) != 2:
        print(json.dumps({"error": "Usage: python whisper-turbo-transcribe.py <media_url>"}))
        sys.exit(1)
    ...
    try:
        ...
        result = transcribe_with_whisper_turbo(audio_path)
        ...
        error_response = {
            "error": f"Whisper turbo transcription failed: {str(e)}"
        }
