...
def warp(img, flow):
    ...
    output = F.grid_sample(input=img, grid=grid_, mode='bilinear', padding_mode=pd, align_corners=True)
    return output
...
class IFRNet(nn.Module):
    ...
    def forward(self, img0, img1, embt
        ...
        up_flow0_1 = resize(up_flow0_1, scale_factor=(1.0/self.scale_factor)) * (1.0/self.scale_factor)
        up_flow1_1 = resize(up_flow1_1, scale_factor=(1.0/self.scale_factor)) * (1.0/self.scale_factor)
