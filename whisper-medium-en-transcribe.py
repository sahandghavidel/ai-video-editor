#!/usr/bin/env python3
"""
Whisper Medium.en transcription script that can be called from Node.js
Takes a media URL as argument and returns JSON transcription
Uses the medium.en Whisper model - English-only, ~5GB VRAM, ~2x speed
"""
...
def transcribe_with_whisper_medium_en(audio_path: str) -> dict:
    """Transcribe audio using Whisper medium.en model (English-only)"""
    ...
    try:
        ...
        # Load the medium.en model (English-only, better than small for English)
...
def main():
    if len(sys.argv) != 2:
        print(json.dumps({"error": "Usage: python whisper-medium-en-transcribe.py <media_url>"}))
        sys.exit(1)
    ...
    try:
        ...
        # Transcribe with Whisper medium.en
        result = transcribe_with_whisper_medium_en(audio_path)
        ...
        error_response = {
            "error": f"Whisper medium.en transcription failed: {str(e)}"
        }
