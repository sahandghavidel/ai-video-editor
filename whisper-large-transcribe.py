#!/usr/bin/env python3
"""
Whisper Large transcription script that can be called from Node.js
Takes a media URL as argument and returns JSON transcription
Uses the large-v3 Whisper model for maximum accuracy
"""
...
def transcribe_with_whisper_large(audio_path: str) -> dict:
    ...
    try:
        ...
        # Load the large model (large-v3, highest accuracy)
...
def main():
    if len(sys.argv) != 2:
        print(json.dumps({"error": "Usage: python whisper-large-transcribe.py <media_url>"}))
        sys.exit(1)
    ...
    try:
        ...
        # Transcribe with Whisper large
        result = transcribe_with_whisper_large(audio_path)
        ...
        error_response = {
            "error": f"Whisper large transcription failed: {str(e)}"
        }
