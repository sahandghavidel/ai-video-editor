...
export async function optimizeSilence(
  ...
  try {
    ...
    // Get video properties to match exactly
    const videoInfoCommand = `ffprobe -v error -select_streams v:0 -show_entries stream=r_frame_rate,width,height,pix_fmt -of default=noprint_wrappers=1 "${inputUrl}"`;
