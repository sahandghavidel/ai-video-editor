...
export const ImageOverlayModal: React.FC<ImageOverlayModalProps> = ({
  ...
  const transcriptionFetchSeqRef = useRef(0);
    ...
    try {
      ...
      const extractUrl = (raw: unknown): string => {
        ...
        if (typeof raw === 'object') {
          ...
          const url =
            obj.url ?? (obj.file as { url?: unknown } | undefined)?.url;
      ...
      // don't re-run OCR.
      {
        ...
        if (isFresh && parsed.hasText !== null) {
          setSceneHasTextStatus(
            parsed.hasText
              ? 'Already checked: hasText (7099)=true for this Image for Scene (7094)'
              : 'Already checked: hasText (7099)=false for this Image for Scene (7094)',
          );
      ...
      const res = await fetch('/api/detect-text-in-image?accurate=1', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ imageUrl: imageUrl7094 }),
      });
      ...
      {
        const url = imageUrl7094;
        const value = `${hasText ? 'true' : 'false'}|${url}`;

        const patchRes = await fetch(`/api/baserow/scenes/${sceneId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          // field_7099 is a single-line text value like: "true|<imageUrl>".
          body: JSON.stringify({ field_7099: value }),
        });
      ...
      if (hasText) {
        setSceneHasTextStatus(
          method
            ? `Small text detected (${method}) → hasText (7099)=true (Saved)`
            : 'Small text detected → hasText (7099)=true (Saved)',
        );
      } else {
        const c =
        ...
        setSceneHasTextStatus(
          snippet.trim()
            ? `No small text detected${c}${m}${fontNote}${r} → hasText (7099)=false (Saved) · “${snippet.trim()}”`
            : `No small text detected${c}${m}${fontNote}${r} → hasText (7099)=false (Saved)`,
        );
      }
  ...
  const withCacheBust = useCallback((url: string | null) => {
    ...
    // If this looks like a signed URL (S3/MinIO/GCS), adding query params can
    // invalidate the signature. In that case, rely on cache: 'no-store' only.
    ...
    if (isOpen && sceneId) {
      const fetchTranscription = async () => {
        try {
          ...
          let currentScene = getLocalSceneSnapshot();
          let captionsUrl = resolveCaptionsUrl(currentScene);
          let captionIdentity = getFinalVideoCaptionStatus({
            sceneId,
            finalVideoUrl: getSceneStringField(currentScene, 'field_6886'),
          ...
          if (captionIdentity.status !== 'matched') {
            ...
            currentScene = latestSceneData ?? currentScene;
            captionsUrl = resolveCaptionsUrl(latestSceneData);
            captionIdentity = getFinalVideoCaptionStatus({
              sceneId,
              finalVideoUrl: getSceneStringField(
                currentScene,
                'field_6886',
              ),
              captionsUrl: captionsUrl ?? '',
          ...
          if (captionsUrl) {
            const response = await fetch(withCacheBust(captionsUrl as string), {
              cache: 'no-store',
            });
          ...
          <div className='flex items-center gap-4'>
            <div className='flex items-center gap-2'>
              ...
              <button
                ...
                className='px-3 py-1 text-sm font-medium bg-gray-100 hover:bg-gray-200 rounded disabled:opacity-50 disabled:cursor-not-allowed'
                title='Detect text in Image for Scene (7094) and save hasText (7099) as "true|<imageUrl>" or "false|<imageUrl>" (stale-safe). If 7099 is already fresh for the current 7094 URL, this will not re-run OCR.'
              >
                {isDetectingSceneImageText ? (
                  <span className='inline-flex items-center gap-2'>
                    <Loader2 className='h-4 w-4 animate-spin' />
