...
export default function SceneCard({
  ...
  const [showRecentlyModifiedTTS, setShowRecentlyModifiedTTS] =
    useState<boolean>(false);
  const [showTextTtsFilters, setShowTextTtsFilters] = useState(false);
  const textTtsFiltersRef = useRef<HTMLDivElement>(null);
  ...
  const [hyperFramesStatusSceneId, setHyperFramesStatusSceneId] = useState<number | null>(null);
  const hyperFramesMenuRef = useRef<HTMLDivElement>(null);
  const hyperFramesMenuTriggerRef = useRef<HTMLButtonElement | null>(null);
  const hyperFramesActionLockRef = useRef(false);
  ...
  useEffect(() => {
    if (!showTextTtsFilters) return;

    const closeOnOutsideClick = (event: MouseEvent) => {
      if (!textTtsFiltersRef.current?.contains(event.target as Node)) {
        setShowTextTtsFilters(false);
      }
    };
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') setShowTextTtsFilters(false);
    };
  ...
  const closeHyperFramesMenu = () => {
    setHyperFramesMenu(null);
    hyperFramesMenuTriggerRef.current?.focus();
  };
  ...
  useEffect(() => {
    if (!hyperFramesMenu || hyperFramesMenu.loading) return;
    hyperFramesMenuRef.current?.querySelector<HTMLButtonElement>('button[role="menuitem"]:not(:disabled)')?.focus();
  }, [hyperFramesMenu]);

  const openHyperFramesMenu = async (sceneId: number, x: number, y: number, trigger: HTMLButtonElement) => {
    hyperFramesMenuTriggerRef.current = trigger;
    setHyperFramesMenu({ sceneId, x, y, scene: null, loading: true });
    setHyperFramesActionStatus((prev) => ({ ...prev, [sceneId]: '' }));
    try {
      const scene = await getSceneById(sceneId);
      if (!scene) throw new Error('Scene could not be found');
      setHyperFramesMenu((current) => current?.sceneId === sceneId ? { ...current, scene, loading: false } : current);
    } catch (error) {
      setHyperFramesMenu((current) => current?.sceneId === sceneId ? { ...current, loading: false } : current);
      setHyperFramesActionStatus((prev) => ({ ...prev, [sceneId]: error instanceof Error ? error.message : 'Could not load scene' }));
    }
  };
  ...
  const runHyperFramesMenuAction = async (
    ...
    if (hyperFramesActionLockRef.current) return;
    hyperFramesActionLockRef.current = true;
    setHyperFramesStatusSceneId(sceneId);
    setHyperFramesActionSceneId(sceneId);
    const actionNames = {
      prompt: 'HF prompt', html: 'HF HTML', render: 'HF video',
      apply: 'Applied HF video', deleteHtml: 'HF HTML', deleteVideo: 'HF video URL',
    };
    setHyperFramesActionStatus((prev) => ({ ...prev, [sceneId]: `${actionNames[action]}: checking scene...` }));
    try {
      const latestScene = await getSceneById(sceneId);
      if (!latestScene) throw new Error('Scene could not be found');
      const prompt = extractFieldValueAsText(latestScene.field_7365).trim();
      const html = extractFieldValueAsText(latestScene.field_7367).trim();
      const video = extractFieldValueAsText(latestScene.field_7368).trim();
      const finalVideo = extractFieldValueAsText(latestScene.field_6886).trim();
      if (action === 'prompt' && !finalVideo) throw new Error('There is no Final video for the HF prompt');
      if (action === 'html' && !prompt) throw new Error('Generate an HF prompt first');
      if ((action === 'render' || action === 'deleteHtml') && !html) throw new Error('There is no saved HF HTML');
      if ((action === 'apply' || action === 'deleteVideo') && !video) throw new Error('There is no saved HF video');
      if (action === 'apply' && !finalVideo) throw new Error('There is no Final video to apply HF to');
      const replacement = action === 'prompt' ? prompt : action === 'html' ? html : action === 'render' ? video : '';
      if (replacement && !window.confirm(`Replace the existing ${actionNames[action]} for scene ${sceneId}?`)) {
        setHyperFramesActionStatus((prev) => ({ ...prev, [sceneId]: '' }));
        return;
      }
      if ((action === 'deleteHtml' || action === 'deleteVideo') &&
        !window.confirm(action === 'deleteHtml'
          ? `Clear the saved HyperFrames HTML for scene ${sceneId}?`
          : `Clear the saved HyperFrames video URL for scene ${sceneId}? The uploaded file will remain in storage.`)) {
        setHyperFramesActionStatus((prev) => ({ ...prev, [sceneId]: '' }));
        return;
      }
      setHyperFramesActionStatus((prev) => ({ ...prev, [sceneId]: `${actionNames[action]}: ${action.startsWith('delete') ? 'clearing' : 'working'}...` }));
      ...
      if (action === 'prompt') {
        ...
        if (result?.skipped) throw new Error(`${actionNames[action]} was already applied or skipped`);
      ...
      setHyperFramesMenu((current) => current?.sceneId === sceneId && current.scene
        ? { ...current, scene: { ...current.scene, ...changes } } : current);
      setHyperFramesActionStatus((prev) => ({ ...prev, [sceneId]: `${actionNames[action]} ${action.startsWith('delete') ? 'cleared' : 'saved'} for scene ${sceneId}.` }));
    } catch (error) {
      setHyperFramesActionStatus((prev) => ({
        ...prev, [sceneId]: `${actionNames[action]} failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      }));
    } finally {
      hyperFramesActionLockRef.current = false;
  ...
  return (
    <div className='w-full'>
            ...
            <div className='flex flex-col sm:flex-row sm:flex-wrap gap-1.5 sm:gap-2'>
              ...
              <div className='flex flex-wrap items-center gap-1'>
                ...
                <div ref={textTtsFiltersRef} className='relative'>
                  <button
                    type='button'
                    onClick={() => setShowTextTtsFilters(!showTextTtsFilters)}
                    aria-expanded={showTextTtsFilters}
                    aria-haspopup='true'
                    aria-label={`Text and TTS filters${showOnlyEmptyText || showOnlyNotEmptyText || showRecentlyModifiedTTS ? ', active' : ''}`}
                    title='Text and TTS filters'
                    className={`inline-flex h-7 w-7 items-center justify-center rounded-full transition-colors ${
                      showOnlyEmptyText || showOnlyNotEmptyText || showRecentlyModifiedTTS
                  ...
                  {showTextTtsFilters && (
                    <div className='absolute left-0 top-full z-50 mt-1 w-44 rounded-lg border border-gray-200 bg-white p-2 text-[11px] text-gray-700 shadow-lg'>
                      <div className='mb-1 font-medium text-gray-500'>Text</div>
                      {[
                        { label: 'All', value: 'all' },
                        { label: 'Empty sentence', value: 'empty' },
                        { label: 'Original available', value: 'original' },
                      ].map((option) => (
                        <label key={option.value} className='flex cursor-pointer items-center gap-2 rounded px-1 py-1 hover:bg-gray-100'>
                          <input
                            type='radio'
                            name='scene-text-filter'
                            checked={option.value === (showOnlyEmptyText ? 'empty' : showOnlyNotEmptyText ? 'original' : 'all')}
                            onChange={() => {
                              setShowOnlyEmptyText(option.value === 'empty');
                              setShowOnlyNotEmptyText(option.value === 'original');
                            }}
                          />
                          {option.label}
                        </label>
                      ...
                      <label className='flex cursor-pointer items-center gap-2 rounded px-1 py-1 hover:bg-gray-100' title='TTS audio modified in the last 10 days'>
                        <input
                          type='checkbox'
                          checked={showRecentlyModifiedTTS}
                          onChange={() => setShowRecentlyModifiedTTS(!showRecentlyModifiedTTS)}
                        />
                        Recent TTS (10 days)
                      </label>
                    </div>
                      ...
                      <>
                        ...
                        <button
                          ...
                          onContextMenu={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            void openHyperFramesMenu(
                              scene.id,
                              Math.min(e.clientX, window.innerWidth - 248),
                              Math.min(e.clientY, window.innerHeight - 448),
                              e.currentTarget,
                            );
                          }}
                          aria-label={
      ...
      {hyperFramesMenu && createPortal((() => {
        const scene = hyperFramesMenu.scene;
        ...
        const items: Array<{ label: string; detail: string; action: 'prompt' | 'html' | 'render' | 'apply' | 'deleteHtml' | 'deleteVideo'; disabled?: boolean }> = [
          { label: prompt ? 'Regenerate HF Prompt' : 'HF Prompt', detail: !finalVideo ? 'Requires Final video' : prompt ? 'Replaces saved prompt' : 'Create a scene prompt', action: 'prompt', disabled: !finalVideo },
          { label: html ? 'Regenerate HF HTML' : 'HF HTML', detail: !prompt ? 'Requires HF Prompt' : html ? 'Replaces saved HTML' : 'Generate from prompt', action: 'html', disabled: !prompt },
          { label: video ? 'Rerender HF' : 'Render HF', detail: !html ? 'Requires HF HTML' : video ? 'Replaces saved HF video' : 'Render saved HTML', action: 'render', disabled: !html },
          { label: 'Apply HF', detail: !video ? 'Requires HF video' : !finalVideo ? 'Requires Final video' : 'Apply to Final video', action: 'apply', disabled: !video || !finalVideo },
          { label: 'Delete HTML', detail: html ? 'Clear saved HTML field' : 'No saved HTML', action: 'deleteHtml', disabled: !html },
          { label: 'Delete HF Video', detail: video ? 'Clear saved video URL' : 'No saved HF video', action: 'deleteVideo', disabled: !video },
        ];
        return <div
          ref={hyperFramesMenuRef}
          role='menu'
          aria-label={`HyperFrames actions for scene ${sceneId}`}
          className='fixed z-[100] max-h-[90vh] w-60 overflow-y-auto rounded-lg border border-gray-200 bg-white p-1 shadow-xl'
          style={{ left: Math.max(8, hyperFramesMenu.x), top: Math.max(8, hyperFramesMenu.y) }}
          onContextMenu={(event) => event.preventDefault()}
          onKeyDown={(event) => {
            if (!['ArrowDown', 'ArrowUp', 'Home', 'End'].includes(event.key)) return;
            const buttons = Array.from(hyperFramesMenuRef.current?.querySelectorAll<HTMLButtonElement>('button[role="menuitem"]:not(:disabled)') ?? []);
        ...
        >
          <p className='px-3 py-2 text-xs font-semibold text-gray-500'>HyperFrames · Scene {sceneId}</p>
          {hyperFramesMenu.loading && <p className='px-3 py-2 text-sm text-gray-600'>Checking scene...</p>}
          {!hyperFramesMenu.loading && scene && <>
          {items.slice(0, 3).map((item) => <button key={item.action} type='button' role='menuitem'
            ...
            className='block w-full rounded px-3 py-2 text-left text-sm text-gray-800 hover:bg-violet-50 disabled:cursor-not-allowed disabled:opacity-60'
          ><span className='block'>{item.label}</span><span className='block text-xs text-gray-500'>{item.detail}</span></button>)}
          <button type='button' role='menuitem' disabled={busy || !html}
            onClick={() => { setHyperFramesEditorSceneId(sceneId); closeHyperFramesMenu(); }}
            className='block w-full rounded px-3 py-2 text-left text-sm text-gray-800 hover:bg-violet-50 disabled:cursor-not-allowed disabled:opacity-60'
          ><span className='block'>Edit HF</span><span className='block text-xs text-gray-500'>{html ? 'Open visual editor' : 'Requires HF HTML'}</span></button>
          {items.slice(3).map((item) => <button key={item.action} type='button' role='menuitem'
            disabled={busy || item.disabled}
            onClick={() => { void runHyperFramesMenuAction(sceneId, item.action); }}
            className={`block w-full rounded px-3 py-2 text-left text-sm hover:bg-violet-50 disabled:cursor-not-allowed disabled:opacity-60 ${item.action.startsWith('delete') ? 'text-red-700' : 'text-gray-800'}`}
          ><span className='block'>{item.label}</span><span className='block text-xs text-gray-500'>{item.detail}</span></button>)}
          </>}
      ...
      {hyperFramesStatusSceneId !== null && hyperFramesActionStatus[hyperFramesStatusSceneId] && !hyperFramesMenu && createPortal(
        <div role='status' className='fixed bottom-4 right-4 z-[100] flex max-w-sm items-start gap-3 rounded-lg border border-gray-200 bg-white p-3 text-sm text-gray-800 shadow-xl'>
          <span>{hyperFramesActionStatus[hyperFramesStatusSceneId]}</span>
          <button type='button' onClick={() => setHyperFramesStatusSceneId(null)} aria-label='Dismiss HyperFrames status' className='text-gray-500 hover:text-gray-800'>×</button>
        </div>, document.body)}
