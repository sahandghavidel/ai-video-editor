...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `[COHERE_LOCAL] Starting transcription with ${pythonCommand} (${pythonSource})`,
    );
    ...
    const transcriptionPromise: Promise<CohereLocalTranscriptionResult> =
      new Promise((resolve, reject) => {
        ...
        pythonProcess.on('close', (code) => {
          if (code === 0) {
            try {
              ...
              reject(
                new Error(
                  `Failed to parse Cohere local transcription JSON: ${String(parseError)}`,
                ),
              );
            }
          } else {
            console.warn(
              `[COHERE_LOCAL] Cohere failed, trying whisper-small fallback...`,
            );
            runWhisperSmallFallback(String(media_url))
              ...
              .catch((fallbackError) => {
                reject(
                  new Error(
                    `Cohere local transcription failed with code ${code}: ${stderr || stdout}. Fallback also failed: ${fallbackError instanceof Error ? fallbackError.message : String(fallbackError)}`,
                  ),
                );
              });
          }
        ...
        pythonProcess.on('error', (error) => {
          reject(
            new Error(
              `Failed to start Cohere local transcription process: ${error.message}`,
            ),
          );
        });
      });
    ...
    console.error('[COHERE_LOCAL] transcribe-video error:', error);
    return NextResponse.json(
      {
        error: 'Failed to transcribe video with Cohere local model',
