...
export async function POST(request: NextRequest) {
  try {
    ...
    if (model === 'cohere-local') {
      ...
      transcriptionPromise = new Promise((resolve, reject) => {
        ...
        pythonProcess.on('close', (code) => {
          if (code === 0) {
            ...
            reject(
              new Error(
                `Whisper turbo transcription failed with code ${code}: ${stderr}`,
              ),
            );
          }
        });
        ...
        pythonProcess.on('error', (error) => {
          reject(
            new Error(
              `Failed to start Whisper turbo transcription process (${pythonSource}): ${error.message}`,
            ),
          );
        });
      });
      ...
      transcriptionPromise = new Promise((resolve, reject) => {
        ...
        pythonProcess.on('close', (code) => {
          if (code === 0) {
            ...
            reject(
              new Error(
                `Whisper large transcription failed with code ${code}: ${stderr}`,
              ),
            );
          }
        });
        ...
        pythonProcess.on('error', (error) => {
          reject(
            new Error(
              `Failed to start Whisper large transcription process (${pythonSource}): ${error.message}`,
            ),
          );
        });
      });
      ...
      transcriptionPromise = new Promise((resolve, reject) => {
        ...
        pythonProcess.on('close', (code) => {
          if (code === 0) {
            ...
            reject(
              new Error(
                `Whisper medium.en transcription failed with code ${code}: ${stderr}`,
              ),
            );
          }
        });
        ...
        pythonProcess.on('error', (error) => {
          reject(
            new Error(
              `Failed to start Whisper medium.en transcription process (${pythonSource}): ${error.message}`,
            ),
          );
        });
      });
      ...
      console.log(`Using WhisperX python: ${pythonCommand} (${pythonSource})`);
      ...
      transcriptionPromise = new Promise((resolve, reject) => {
        ...
        pythonProcess.on('close', (code) => {
          if (code === 0) {
            ...
            reject(
              new Error(
                `WhisperX transcription failed with code ${code}: ${stderr}`,
              ),
            );
          }
        });
        ...
        pythonProcess.on('error', (error) => {
          reject(
            new Error(
              `Failed to start WhisperX transcription process (${pythonSource}): ${error.message}`,
            ),
          );
        });
      });
      ...
      console.log(
        `Using MLX WhisperX python: ${pythonCommand} (${pythonSource})`,
      );
      ...
      transcriptionPromise = new Promise((resolve, reject) => {
        ...
        pythonProcess.on('error', (error) => {
          reject(
            new Error(
              `Failed to start MLX WhisperX transcription process (${pythonSource}): ${error.message}`,
            ),
          );
        });
      });
